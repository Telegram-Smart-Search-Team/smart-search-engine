...
def _get_cfg_for_event(event: events.NewMessage.Event) -> dict[str, tp.Any]:
...
async def fetch_message_by_id(
    ...
    try:
        ...
        logging.exception(f"[ERROR] fetch_message_by_id peer={peer_id} msg={message_id}", exc_info=True)
        return None
...
async def forward_message_by_id(
    ...
    try:
        ...
        if not msg:
            logging.warning("[WARN] forward_message_by_id: source message not found")
            return
...
async def process_message_graphs(client: TelegramClient, db_message_id: int, chat_id: int, message_id: int):
    ...
    for graph_size in GRAPH_SIZES:
        try:
            ...
            logging.info(f"Stored graph embedding for message {message_id}, size {actual_size}", flush=True)

            previous_graph_size = graph_size

        except Exception:
            logging.exception(f"Error processing graph size {graph_size} for message {message_id}", exc_info=True)
            traceback.print_exc()
    ...
    while True:
        try:
            ...
            try:
                ...
                logging.exception("[ERROR] watch_forward_queue: invalid JSON", exc_info=True)
                continue
            ...
            logging.exception("[ERROR] watch_forward_queue loop", exc_info=True)
...
async def handle_processed_message(client: TelegramClient, payload: dict[str, tp.Any]):
    ...
    try:
        ...
        logging.exception("Error storing message or processing graphs", exc_info=True)
...
async def main() -> None:
    ...
    async def handle_new_message(event: events.NewMessage.Event):
        ...
        try:
            ...
            logging.exception("[ERROR] building payload for NewMessage", exc_info=True)
            return
    ...
    async def handle_album(event: events.Album.Event):
        try:
            ...
            logging.exception("[ERROR] building payload for Album", exc_info=True)
            return
